...
import com.sky.kafka.configurator.config.{Config, ConfigParsing}
...
class ConfigParsingSpec extends BaseSpec {
  ...
  it should "support reading Kafka config from sys env" in new TestContext {
    ...
    runParser(
      args,
      Map(supportEnvVarKeyFrom(validConfigKey) -> someConfigValue)
  ...
  it should "only support sys env values prefixed with KAFKA_" in new TestContext {
    ...
    runParser(
      args,
      Map(supportEnvVarKeyFrom(validConfigKey) -> someConfigValue, "ssl.keystore.password" -> "foo")
    ).success.value shouldBe expectedAppConfig(Map(validConfigKey -> someConfigValue))
  }

  it should "drop properties/env vars that are not valid Kafka admin client configs" in new TestContext {
    val args =
      buildArgsFrom(configFilePath, BootstrapServers, Map(validConfigKey -> someConfigValue, "invalid" -> "invalid"))

    runParser(
      args,
      Map("KAFKA_CLIENT_ID" -> someConfigValue, "another-invalid" -> "invalid")
  ...
  private class TestContext {
    ...
    def runParser(args: Array[String], props: Map[String, String]): Try[Config] =
      captureOutErr {
    ...
    def expectedAppConfig(props: Map[String, String]): Config =
      Config(Seq(new File(configFilePath)), BootstrapServers, props)
  }
