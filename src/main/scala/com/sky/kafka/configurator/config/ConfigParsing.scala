...
object ConfigParsing {
  ...
  private val parser = new OptionParser[Config]("kafka-configurator") {
    opt[Seq[File]]('f', "files")
      ...
      .validate(files =>
        if (files.forall(_.exists)) success else failure(s"One of ${files.mkString(",")} does not exist.")
      )

    opt[String]("bootstrap-servers")
      .required()
  ...
  def parse(args: Seq[String], envVars: Map[String, String]): Try[Config] =
    parser
      .parse(args, Config())
      .map { appConfig =>
